...
from worker import OCRWorker, TranslateWorker, TextRegionDetectWorker

parent = os.path.dirname(os.path.dirname(os.path.realpath(__file__)))
sys.path.append(parent)
from utils.misc import pad_image_to_square
...
class Controller(QObject):
    ...
    def init_worker(self, trd_path, ocr_path, translate_path, beam_size, source_lang, target_lang):
        ...
        self.translate_worker = TranslateWorker(translate_path, beam_size, source_lang, target_lang)
    ...
    def ocr_process(self, data, box_xyxy):
        if self.ocr_worker is not None:
            print("OCR process")
            self.ocr_id += 1
